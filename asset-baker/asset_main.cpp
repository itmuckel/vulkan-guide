...
bool convert_image(const fs::path& input, const fs::path& output)
{
	...
	std::cout << "png took " << std::chrono::duration_cast<std::chrono::nanoseconds>(diff).count() / 1000000.0 << "ms"
		<< std::endl;
...
void pack_vertex(assets::Vertex_f32_PNCV& new_vert, tinyobj::real_t vx, tinyobj::real_t vy, tinyobj::real_t vz,
                 tinyobj::real_t nx, tinyobj::real_t ny, tinyobj::real_t nz, tinyobj::real_t ux, tinyobj::real_t uy)
{
...
void pack_vertex(assets::Vertex_P32N8C8V16& new_vert, tinyobj::real_t vx, tinyobj::real_t vy, tinyobj::real_t vz,
                 tinyobj::real_t nx, tinyobj::real_t ny, tinyobj::real_t nz, tinyobj::real_t ux, tinyobj::real_t uy)
{
	new_vert.position[0] = vx;
	new_vert.position[1] = vy;
	...
	new_vert.normal[0] = static_cast<uint8_t>(((nx + 1.0f) / 2.0f) * 255);
	new_vert.normal[1] = static_cast<uint8_t>(((ny + 1.0f) / 2.0f) * 255);
	new_vert.normal[2] = static_cast<uint8_t>(((nz + 1.0f) / 2.0f) * 255);
...
template <typename V>
void extract_mesh_from_obj(std::vector<tinyobj::shape_t>& shapes, tinyobj::attrib_t& attrib,
                           std::vector<uint32_t>& _indices, std::vector<V>& _vertices)
{
	// Loop over shapes
...
bool convert_mesh(const fs::path& input, const fs::path& output)
{
	...
	std::cout << "obj took " << std::chrono::duration_cast<std::chrono::nanoseconds>(diff).count() / 1000000.0 << "ms"
		<< std::endl;
...
std::string calculate_assimp_material_name(const aiScene* scene, int materialIndex)
{
	...
	itoa(materialIndex, buffer, 10);
	std::string matname = "MAT_" + std::string{buffer} + "_" + std::string{
		scene->mMaterials[materialIndex]->GetName().C_Str()
	};
	return matname;
}

void extract_assimp_materials(const aiScene* scene, const fs::path& input, const fs::path& outputFolder,
                              const ConverterState& convState)
{
...
void extract_assimp_nodes(const aiScene* scene, const fs::path& input, const fs::path& outputFolder,
                          const ConverterState& convState)
{
	assets::PrefabInfo prefab;
	...
	uint64_t lastNode = 0;
	std::function<void(aiNode* node, aiMatrix4x4& parentmat, uint64_t)> process_node = [&
		](aiNode* node, aiMatrix4x4& parentmat, uint64_t parentID)
	{
		...
		for (auto& p : fs::recursive_directory_iterator(directory))
		{
			...
			if (false)
			{
				...
				const aiScene* scene;
				{
					...
					scene = importer.ReadFile(p.path().string(),
					                          aiProcess_OptimizeMeshes | aiProcess_GenNormals | aiProcess_FlipUVs);
					//aiProcess_Triangulate | aiProcess_OptimizeMeshes | aiProcess_FlipUVs | aiProcess_GenNormals | aiProcess_GenBoundingBoxes);
