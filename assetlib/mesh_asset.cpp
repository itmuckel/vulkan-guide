...
assets::VertexFormat parseFormat(const std::string& value)
{
	if (value == "PNCV_F32")
	{
...
assets::MeshInfo assets::readMeshInfo(AssetFile* file)
{
	...
	info.indexSize = static_cast<uint8_t>(metadata["index_size"]);
	info.originalFile = metadata["original_file"];

	const std::string compressionString = metadata["compression"];
	...
	info.vertexFormat = parseFormat(vertexFormat);

	return info;
}

void assets::unpackMesh(MeshInfo* info, const char* sourcebuffer, size_t sourceSize, char* vertexBufer,
                        char* indexBuffer)
{
	...
	LZ4_decompress_safe(sourcebuffer, decompressedBuffer.data(), static_cast<int>(sourceSize),
...
assets::AssetFile assets::packMesh(MeshInfo* info, char* vertexData, char* indexData)
{
	...
	const size_t fullsize = info->vertexBuferSize + info->indexBuferSize;

	std::vector<char> mergedBuffer{};
	mergedBuffer.resize(fullsize);

	//copy vertex buffer
	memcpy(mergedBuffer.data(), vertexData, info->vertexBuferSize);

	//copy index buffer
	memcpy(mergedBuffer.data() + info->vertexBuferSize, indexData, info->indexBuferSize);
	...
	const int compressedSize = LZ4_compress_default(mergedBuffer.data(), file.binaryBlob.data(),
